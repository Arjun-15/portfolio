import { createAsyncThunk, createSlice } from "@reduxjs/toolkit";

interface Portfolio {
    projects: [];
    project: {};
}
const InitialState: Portfolio = {
    projects: [],
    project: {}
}

const PortfolioSlice = createSlice({
    name: 'portfolio',
    initialState: InitialState,
    reducers: {
        // addProjects: (state: any, action: any) => {
        //     //
        // }
    },
    extraReducers(builder) {
        builder.addCase(fetchProjects.fulfilled, (state: any, action: any) => {
            state.projects = action.payload;
        })
            .addCase(fetchProject.fulfilled, (state: any, action: any) => {
                state.project = action.payload;
                // console.log(action.payload)
            })
    }
});

export const portfolioReducer = PortfolioSlice.reducer;
export const portfolioActions = PortfolioSlice.actions;
export const portfolioSelector = (state: any) => state.portfolioReducer;


export const fetchProjects = createAsyncThunk("portfolio/fetchProjects", () => myProjects)
export const fetchProject = createAsyncThunk("portfolio/fetchProject", (payload:any) => {
    if (payload) {
        const data = myProjects.find((x: any) => x.id == payload.projectId)
        // console.log(myProjects, payload.projectId);
        return data;
    }
});


////////------------extra -------------
//project
const myProjects = [
    {
        id: 1,
        image: 'data:image/jpeg;base64,',
        title: 'Crypto-Tracker',
        description: 'Crypto-Tracker is a cutting-edge web application designed to provide users with real-time information and updates on various cryptocurrencies. Built using React and TypeScript, this project leverages modern web development practices to create a responsive and user-friendly interface. The application integrates with external APIs to fetch and display live data, including current prices, market trends, and historical charts for a wide range of cryptocurrencies.',
        keyfeatures: [
            { key:"Real-Time Data Fetching", value:"Utilizes REST APIs to gather real-time data on cryptocurrency prices, market cap, volume, and more." },
            { key:"User Authentication", value:"Implements secure user authentication allowing users to create accounts, log in, and save their favorite cryptocurrencies." },
            { key:"Favorite Coins", value:"Users can mark specific cryptocurrencies as favorites and receive personalized updates." },
            { key:"Search and Filter", value:"Provides robust search and filter functionalities to help users find specific cryptocurrencies quickly." },
            { key:"Responsive Design", value:"Ensures a seamless experience across various devices, including desktops, tablets, and mobile phones." },
            { key:"Chart Visualization", value:"Offers interactive charting tools to visualize historical data and market trends." },
        ],
        VariablesAndElaborations: [
            {
                title: "API_BASE_URL",
                desc: `const API_BASE_URL = 'https://api.coingecko.com/api/v3/coins/';`,
                Explanation: "The base URL for the external cryptocurrency API used to fetch data."
            },

            {
                title: "CryptoState Interface",
                desc:
                    `'typescript'
                interface CryptoState {
                  coins: Coin[];
                  favouriteCoins: string[];
                  searchResults: Coin[];
                  selectedCoin: Coin | null;
                  chartData: ChartData[];
                }
                `,
                Explanation: "Defines the shape of the state used to manage cryptocurrency data within the Redux store."
            },

            {
                title: "Coin Interface",
                desc: `'typescript'
                interface Coin {
                  id: string;
                  name: string;
                  symbol: string;
                  price: number;
                  marketCap: number;
                  volume: number;
                  change: number;
                }
                `
                , Explanation: "Represents the structure of a cryptocurrency object, including its essential attributes."
            },

            {
                title: "Fetch Coins Thunk",
                desc: `'typescript'
                export const fetchCoins = createAsyncThunk('coins/fetchCoins', async () => {
                  const response = await fetch(\`\${API_BASE_URL}/coins\`);
                  return response.json();
                });
                `
                , Explanation: "An asynchronous thunk action for fetching the list of cryptocurrencies from the API."
            },

            {
                title: "Toggle Favourite Action",
                desc: `'typescript'
                export const toggleFavourite = createAction<string>('coins/toggleFavourite');
                `
                , Explanation: "An action to toggle the favorite status of a cryptocurrency, taking the coin ID as a payload."
            },

            {
                title: "Debounce Utility",
                desc: `'typescript'
                let timer: NodeJS.Timeout | null = null;

                const debounce = (func: Function, delay: number) => {
                  if (timer !== null) {
                    clearTimeout(timer);
                  }
                  timer = setTimeout(() => {
                    func();
                  }, delay);
                };
                `
                , Explanation: "A utility function to debounce rapid function calls, ensuring efficient API requests."
            },
        ],
        url: '/portfolio/coin'
    },
    {
        id: 2,
        image:
            "https://img.icons8.com/external-kiranshastry-lineal-color-kiranshastry/64/000000/external-web-development-coding-kiranshastry-lineal-color-kiranshastry.png",
        title: "Web Application Development",
        description:
            "Designed and developed a full-stack web application using React for the frontend and Node.js for the backend. Implemented RESTful APIs for seamless data exchange and integrated user authentication using JWT. Deployed the application on AWS, ensuring scalability and high availability.",
        technologies: "React, Node.js, Express, MongoDB, AWS",
    },
    {
        id: 3,
        image:
            "https://img.icons8.com/external-justicon-lineal-color-justicon/64/000000/external-app-development-responsive-web-design-justicon-lineal-color-justicon.png",
        title: "Mobile Application Development",
        description:
            "Created a cross-platform mobile application using React Native. Developed features such as real-time messaging, geolocation services, and push notifications. Utilized Firebase for backend services including database management, authentication, and cloud storage.",
        technologies: "React Native, Firebase, Redux, Google Maps API",
    },
    {
        id: 4,
        image:
            "https://img.icons8.com/external-flaticons-flat-flat-icons/64/000000/external-backend-no-code-flaticons-flat-flat-icons.png",
        title: "E-commerce Platform",
        description:
            "Built a comprehensive e-commerce platform with a user-friendly interface and secure payment gateway integration. Implemented features like product listing, shopping cart, order tracking, and user reviews. Optimized the platform for performance and SEO to enhance user experience and search engine ranking.",
        technologies: "Angular, Node.js, Express, MySQL, Stripe API",
    },
];